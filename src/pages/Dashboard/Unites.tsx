...
import { FaTrash, FaEdit, FaRuler, FaSearch, FaSave, FaPlus, FaBalanceScale } from "react-icons/fa";
...
function Unites() {
  ...
  const key = ['unite', session?.user.id];
  const {data:unites,isLoading} = useQuery({ queryKey: key, queryFn:() => uniteService.getByUser(session!.user.id), enabled: !!session })
  ...
  const {mutate:createUnite,isPending:loadingCreate} = useMutation({
   mutationFn: (data) => uniteService.create(data),
   onSuccess: () => {
    close();
...
const {mutate:updateUnite,isPending:loadingUpdate} = useMutation({
 mutationFn:(data:{id:string,data:any}) => uniteService.update(data.id, data.data),
 onSuccess: () => {
  closeU();
  qc.invalidateQueries({queryKey:key});
 }
...
const {mutate:deleteUnite,isPending:loadingDelete} = useMutation({
...
const filtered = (Unite = []) => {
  return Unite?.filter(({ nom }) => {
    if (
      debouncedQuery !== '' &&
...
useEffect(() => {
  ...
  setRecords(filtered(unites).slice(from, to) ?? []);
}, [unites,debouncedQuery,page]);
  ...
  return (
      ...
      >
        <Group mb="xs">
          <Text fw={600} className="text-gray-700 dark:text-gray-200 flex items-center gap-2">
            <FaBalanceScale className="text-orange-500" /> Gestion des unités de mesure
          </Text>
          <Badge size="lg" className="bg-gradient-to-r from-orange-500 to-red-500 text-white">
            {filtered(unites)?.length || 0} unité(s)
          </Badge>
        </Group>
        <Divider mb="md" />
        ...
        <div className="flex justify-between items-center w-full md:w-1/2 mb-5">
          <div className="w-full relative">
            <TextInput
              ...
              className="w-full"
              styles={{ input: { '&:focus': { borderColor: '#FF5D14' } } }}
            />
          </div>
        </div>
    <DataTable
      ...
      columns={[
        { 
          accessor: 'nom', 
          title: <Text fw={600} className="text-gray-700 dark:text-gray-200">Nom de l'unité</Text>,
          textAlign: 'left',
          render: (rowData: any) => (
      ...
      emptyState={
        <div className="flex flex-col items-center justify-center py-10">
          ...
          <Text size="lg" fw={500} className="text-gray-500 dark:text-gray-400">Aucune unité trouvée</Text>
          <Text size="sm" className="text-gray-400 dark:text-gray-500 mt-1 max-w-md text-center">
            Vous n'avez pas encore créé d'unité ou votre recherche ne correspond à aucun résultat.
          </Text>
        </div>
      }
      totalRecords={filtered(unites)?.length}
      recordsPerPage={10}
      page={page}
   ...
   <Drawer 
      opened={opened} 
      onClose={close} 
      title={<div className="flex items-center gap-2"><FaPlus className="text-orange-500" /> Création d'une nouvelle unité</div>}
      position="right"
      overlayProps={{ blur: 3 }}
      size="md"
      classNames={{
    ...
    >
      ...
      <div className="mt-4">
        ...
        >
          <Group mb="xs">
            <Text fw={600} className="text-gray-700 dark:text-gray-200 flex items-center gap-2">
              <FaRuler className="text-orange-500" /> Informations de l'unité
            </Text>
          </Group>
          <Divider mb="md" />
          ...
          <form onSubmit={form.onSubmit(onCreate)} className="space-y-4">
            <TextInput
              ...
              placeholder="Saisissez le nom de l'unité (ex: kg, litre, pièce)"
              leftSection={<FaRuler size={16} />}
              {...form.getInputProps('nom')}
              styles={{ input: { '&:focus': { borderColor: '#FF5D14' } } }}
            />
            
            <Text size="sm" color="dimmed" className="mt-2">
              Les unités de mesure vous permettent de définir comment vos articles sont quantifiés (kg, litre, pièce, etc.).
            </Text>
   ...
   <Drawer 
      ...
      opened={openedU} 
      onClose={closeU} 
      title={<div className="flex items-center gap-2"><FaEdit className="text-orange-500" /> Modification d'une unité</div>}
      overlayProps={{ blur: 3 }}
      size="md"
      classNames={{
        title: "text-lg font-medium text-gray-800 dark:text-gray-200",
    ...
    >
      ...
      <div className="mt-4">
        ...
        >
          <Group mb="xs">
            <Text fw={600} className="text-gray-700 dark:text-gray-200 flex items-center gap-2">
              <FaRuler className="text-orange-500" /> Modifier les informations
            </Text>
          </Group>
          <Divider mb="md" />
